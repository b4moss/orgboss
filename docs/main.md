...
- `ID`：主キー（ユニーク制約、auto increment）
- `Name`：組織名（重複可、同名の企業が存在することを想定）
- `Signature`：組織の一意識別子（**ユニーク制約あり**、ユニークインデックス）
  - 日本の法人の場合：法人番号（13桁の数字）
  - 任意団体や海外組織の場合：ランダム文字列
  - データベースレベルでユニーク制約が設定されており、重複は許可されない
  - `signature`の値から、日本の法人番号（13桁の数字）かランダム文字列かを判別可能
- `CreatedAt`：作成日時
- `UpdatedAt`：更新日時
- `DeletedAt`：削除日時（論理削除対応）

**Signatureの設計意図**：
- 組織名は重複を許容するため、一意に組織を識別するために`Signature`を使用
- 日本の法人番号をベースとした管理を可能にしつつ、日本の法人以外（任意団体、海外組織）にも対応
- `signature`の値（13桁の数字かどうか）により、日本の法人番号とランダム文字列を判別可能
